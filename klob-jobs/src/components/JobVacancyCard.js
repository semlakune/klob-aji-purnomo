...
export default function JobVacancyCard(props) {
    ...
    const idRupiah = Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", });
    return (
        <div className={"w-96 rounded overflow-hidden shadow-lg m-5"}>
            ...
            <div className={"px-6 py-4"}>
                ...
                <div className={"grid grid-cols-2 mb-2"}>
                    <h1>Gaji</h1>
                    <h1 className={"bg-stone-500 p-0.5 text-center rounded text-white"}>{idRupiah.format(jobVacancy.salaryFrom)} - {idRupiah.format(jobVacancy.salaryTo)}</h1>
                </div>
                <div className={"grid grid-cols-2 mb-2"}>
                    <button className={"text-blue-600 text-left"}>Baca Detail</button>
